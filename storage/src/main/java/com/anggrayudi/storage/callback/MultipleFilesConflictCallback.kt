 ...
 * @author Anggrayudi H
 */
public abstract class MultipleFilesConflictCallback
...
constructor(public var uiScope: CoroutineScope = GlobalScope) {
   ...
   */
  @UiThread
  public open fun onInvalidSourceFilesFound(
  ...
  public class InvalidSourceFilesAction(private val continuation: CancellableContinuation<Boolean>) {

    /** @param abort stop the process */
    public fun confirmResolution(abort: Boolean) {
  ...
  public class ParentFolderConflictAction(
    ...
    public fun confirmResolution(resolution: List<ParentConflict>) {
      continuation.resumeWith(Result.success(resolution))
    }
  }

  public class ParentConflict(
    public val source: DocumentFile,
    public val target: DocumentFile,
    public val canMerge: Boolean,
    public var solution: ConflictResolution = ConflictResolution.CREATE_NEW,
  )
}
